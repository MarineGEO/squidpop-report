...
```{r xlsx}
xlsx <- "data/Squidpops_example_BocasDelToro_2018.xlsx" # path to the marinegeo squidpop data entry template
```
...
data_w_coords <- data %>% 
  mutate(timeDeployed=format(timeDeployed, format="%H:%M"), 
         timeCollected=format(timeCollected, format="%H:%M")) %>% # read_excel() doesn't parse time fields properly so let's correct it
  mutate(stakesRemaining1hr = numberDeployed) %>%  # number of stakes remaining @ 1 hr didn't get recorded 
  mutate(percentBaitLost1hr=baitMissing1hr/stakesRemaining1hr, 
         percentBaitLost24hr=baitMissing24hr/stakesRemaining24hr) %>% # calculates the percent of bait lost in each time period
  dplyr::left_join(location, by=c("location"="location")) # uses a left join to add the location information
...
data_w_coords_long <- data_w_coords %>% 
  ...
  gather(percentBaitLost24hr, percentBaitLost1hr, key="time", value="percentLost") %>% # gathe percentBaitLost to a long format
